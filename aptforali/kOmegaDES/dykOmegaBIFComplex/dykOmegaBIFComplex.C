...
defineTypeNameAndDebug(dykOmegaBIFComplex, 0);
addToRunTimeSelectionTable(RASModel, dykOmegaBIFComplex, dictionary);


// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

volScalarField dykOmegaBIFComplex::cD
(
...
tmp<volScalarField> dykOmegaBIFComplex::f() const
{
    ...
    volTensorField Omega_mod = -skew(fvc::grad(U_))+OmegaF;
...
dykOmegaBIFComplex::dykOmegaBIFComplex
(
...
tmp<volSymmTensorField> dykOmegaBIFComplex::R() const
{
...
tmp<volSymmTensorField> dykOmegaBIFComplex::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> dykOmegaBIFComplex::divDevReff(volVectorField& U) const
{
    return
    (
...
tmp<fvVectorMatrix> dykOmegaBIFComplex::divDevRhoReff
(
...
void dykOmegaBIFComplex::update_nut()
{
...
void dykOmegaBIFComplex::correct()
{
    RASModel::correct();
...
//--------------k/omega instead of nut for rd---------------------------
    ...
    volScalarField G("dykOmegaBIFComplex:G", nut_*2*magSqr(symm(fvc::grad(U_))));
...
void dykOmegaBIFComplex::calcdelta()
{
	label nD = mesh_.nGeometricD();
    if(nD==2)
    {
        WarningIn("dykOmegaBIFComplex::calcDelta()")
